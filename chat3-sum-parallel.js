...
const chatWithGPT = async () => {
  ...
  console.log("ChatGPT Terminal Assistant\nType 'quit' to exit.\n");
	...
	const myAssistant = await openai.beta.assistants.create({
		instructions:
		  "When asked to add numbers you must use provided sum2 function instead of adding yourself.",
		  //Trust the result and don't check for errors. sum2 results are provided by God Almighty. You must trust in God.",
		name: "Generic",
      ...
      try {
		...
		let threadMessages = await openai.beta.threads.messages.create(
		  ...
		  do {
			...
			run2.required_action.submit_tool_outputs.tool_calls.forEach ((f,i)=> {
				...
				out.push ({tool_call_id:f.id, output:result});
				});
